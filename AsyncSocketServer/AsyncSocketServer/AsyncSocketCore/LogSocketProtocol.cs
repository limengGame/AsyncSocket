...
namespace AsyncSocketServer
{
    public class LogOutputSocketProtocol : BaseSocketProtocol
    {
            ...
            : base(asyncSocketServer, asyncSocketUserToken)
        {
            ...
            lock (Program.AsyncSocketServ.LogOutputSocketProtocolMgr)
            {
                Program.AsyncSocketServ.LogOutputSocketProtocolMgr.Add(this);
            }
        ...
        public override void Close()
        {
            lock (Program.AsyncSocketServ.LogOutputSocketProtocolMgr)
            {
                Program.AsyncSocketServ.LogOutputSocketProtocolMgr.Remove(this);
            }
        }
    ...
    class LogSocketAppender : log4net.Appender.AppenderSkeleton
    {
        ...
        protected override void Append(LoggingEvent loggingEvent)
        {
            ...
            lock (Program.AsyncSocketServ.LogOutputSocketProtocolMgr)
            {
                for (int i = 0; i < Program.AsyncSocketServ.LogOutputSocketProtocolMgr.Count(); i++)
                {
                    Program.AsyncSocketServ.LogOutputSocketProtocolMgr.ElementAt(i).LogFixedBuffer.WriteBuffer(tmpBuffer);
                    Program.AsyncSocketServ.LogOutputSocketProtocolMgr.ElementAt(i).InitiativeSend();
                }
            }
        }
    }
